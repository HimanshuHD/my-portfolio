...
function isCubieInLayer(cubie, definition) {
  const axisIndex = definition.axis === 'x'
    ? 0
    : definition.axis === 'y'
      ? 1
      : 2;
...
function AnimatedLayer({
  ...
  useFrame((_, delta) => {
    if (!layerRef.current || completedRef.current) {
      return;
    }

    elapsedRef.current += delta;
    const progress = Math.min(
      elapsedRef.current / (totalDuration / 1000),
      1,
    );
...
function GameCube({
  ...
  if (activeMove) {
    ...
    animatedCubies = cubies.filter((cubie) =>
      isCubieInLayer(cubie, definition),
    );
    staticCubies = cubies.filter((cubie) =>
      !isCubieInLayer(cubie, definition),
    );
  }
...
function FaceDragHandler({
  ...
  useEffect(() => {
    ...
    return () => {
      if (handlerRef.current === handleStickerPointerDown) {
        handlerRef.current = null;
      }

      if (cancelRef.current === cancelDrag) {
  ...
  useEffect(() => {
    const handlePointerMove = (event) => {
      ...
      if (Math.hypot(dx, dy) < FACE_DRAG_THRESHOLD) {
        return;
      }
      ...
      if (!basis || !groupRef.current) {
        return;
      }

      const quaternion = groupRef.current.getWorldQuaternion(
        new THREE.Quaternion(),
      );
      const rightWorld = new THREE.Vector3(...basis.right).applyQuaternion(
        quaternion,
      );
      const upWorld = new THREE.Vector3(...basis.up).applyQuaternion(
        quaternion,
      );
      const origin = groupRef.current.getWorldPosition(
        new THREE.Vector3(),
      );
      const originPoint = origin.clone().project(drag.camera);
      const rightPoint = origin
        .clone()
        .add(rightWorld)
        .project(drag.camera);
      const upPoint = origin
        .clone()
        .add(upWorld)
        .project(drag.camera);
...
export default function RubiksCube3D({
  ...
  return (
    <div
      ...
      className="rubiks-game-canvas"
    >
      <Canvas
        camera={{
          position: [3.5, 3.1, 4.4],
          fov: 42,
        }}
        shadows
      >
        ...
        <GameCube
          ...
          onStickerPointerDown={(event, sticker) =>
            faceDragHandlerRef.current?.(event, sticker)
          }
          groupRef={cubeGroupRef}
        />
        <OrbitControls
