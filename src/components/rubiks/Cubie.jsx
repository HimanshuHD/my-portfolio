...
const FACE_COLORS = {
  U: '#f4f0d7', R: '#e12d2d', F: '#198b49', D: '#f0d329', L: '#ef7822', B: '#1769aa',
};
...
const STICKER_OFFSET = CUBIE_SIZE / 3;
...
const stickerMaterials = Object.fromEntries(Object.entries(FACE_COLORS).map(([face, color]) => [face, new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 0.08, roughness: 0.38, metalness: 0.02, side: THREE.DoubleSide, polygonOffset: true, polygonOffsetFactor: -1, polygonOffsetUnits: -1 })]));
...
function Sticker({ sticker }) {
  ...
  let rotation = [0, 0, 0];
  if (ny) rotation = [Math.PI / 2, 0, 0]; else if (nx) rotation = [0, Math.PI / 2, 0];
  return <mesh geometry={stickerGeometry} material={material} position={[nx * STICKER_OFFSET, ny * STICKER_OFFSET, nz * STICKER_OFFSET]} rotation={rotation} renderOrder={1} />;
}

export default function Cubie({ position, stickers = [] }) {
  return <group position={position}><mesh geometry={cubieGeometry} material={cubieMaterial} castShadow />{stickers.map((sticker, index) => <Sticker key={`${sticker.color}-${sticker.normal.join(',')}-${index}`} sticker={sticker} />)}</group>;
}
