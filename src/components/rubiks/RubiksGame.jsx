...
export default function RubiksGame({ onClose }) {
  ...
  const scrambleCube = () => {
    ...
    setCube(applyMoves(createSolvedCube(), sequence));
  ...
  const handleMove = (move) => {
    if (isSolved(cube) && !startedAt) return;
    ...
    <section className="rubiks-game-modal" role="dialog" aria-modal="true" aria-labelledby="rubiks-game-title">
      ...
      <div className="rubiks-game-layout">
        ...
        <aside className="rubiks-controls">
          <div className="rubiks-stats"><div><strong>{moves}</strong><span>Moves</span></div><div><strong>{String(Math.floor(elapsed / 60)).padStart(2, '0')}:{String(elapsed % 60).padStart(2, '0')}</strong><span>Time</span></div></div>
          <div className="rubiks-actions"><button type="button" className="button primary" onClick={scrambleCube}>Scramble</button><button type="button" className="button ghost" onClick={resetCube}>Reset</button></div>
          <div className="rubiks-moves"><span>Face turns</span><div>{MOVE_BUTTONS.map((move) => <button key={move} type="button" onClick={() => handleMove(move)}>{move}</button>)}</div></div>
          {scramble.length > 0 && <div className="rubiks-scramble"><span>Scramble</span><p>{scrambleToString(scramble)}</p></div>}
        </aside>
      </div>
    </section>
  </div>;
