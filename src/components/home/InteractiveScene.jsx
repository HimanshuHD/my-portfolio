...
function RubiksCube({ onClick }) {
  ...
  return <group ref={cubeRef} position={[2.6, DESK_TOP + cubieSize / 2 + 0.185, -0.22]} rotation={[0, -0.42, 0]}>
    ...
    <Html position={[-0.86, 0.5, 0]} distanceFactor={6.5} transform={false} zIndexRange={[4, 0]}>
      <div style={{ position: 'relative', width: 175, height: 92, pointerEvents: 'none', userSelect: 'none' }} aria-hidden="true">
        <div style={{ position: 'absolute', left: 0, top: 0, border: '1px solid #293038', background: 'rgba(7,10,12,.72)', backdropFilter: 'blur(10px)', padding: '11px 14px', color: '#b8babc', font: '11px var(--mono)', borderRadius: '10px', whiteSpace: 'nowrap' }}><span style={{ color: accent, marginRight: 7 }}>↔</span>Click to play</div>
        <div style={{ position: 'absolute', left: 108, top: 38, width: 62, height: 1, background: accent, opacity: 0.75, transform: 'rotate(48deg)', transformOrigin: 'left center' }} />
      </div>
    </Html>
