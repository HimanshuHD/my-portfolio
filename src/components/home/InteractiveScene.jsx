...
function Phone() { return <group position={[-1.48, DESK_TOP + 0.024, 1.45]} rotation={[-0.0039, -0.15, -0.00153]}><RoundedBox args={[0.64, 0.05, 1.18]} radius={0.02} smoothness={8} castShadow><meshStandardMaterial color="#171a1c" metalness={0.78} roughness={0.2} /></RoundedBox><mesh position={[0, 0.0258, 0]} rotation={[-Math.PI / 2, 0, 0]}><planeGeometry args={[0.5, 0.96]} /><meshStandardMaterial color="#172c3a" emissive="#0a3842" emissiveIntensity={1.0} /></mesh></group>; }
...
export default function InteractiveScene() { return <div className="interactive-scene"><Canvas shadows dpr={[1, 1.5]} camera={{ position: [7.5, 5.8, 9.5], fov: 38 }}><SceneContent /><OrbitControls enablePan={false} enableZoom={true} zoomSpeed={0.7} minDistance={7} maxDistance={14} autoRotate={false} minPolarAngle={Math.PI / 3.4} maxPolarAngle={Math.PI / 2.05} target={[0, 0.25, 0]} /></Canvas><div className="scene-hint"><span>↔</span> Drag to rotate · Scroll to zoom</div></div>; }
