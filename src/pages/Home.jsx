...
import RubiksGame from '../components/rubiks/RubiksGame';
...
export default function Home() {
  const [rubiksOpen, setRubiksOpen] = useState(false);
  return <>
    <section className="hero hero-3d" aria-labelledby="hero-title">
      <div className="hero-scene-layer" aria-hidden={rubiksOpen}>
        <InteractiveScene onCubeClick={() => setRubiksOpen(true)} />
        <div className="floating-badge badge-one"><Sparkles size={14} /> Interactive by default</div>
      </div>
      <div className="hero-grid"><div className="hero-copy hero-copy-animated"><div className="eyebrow"><span className="dot" /> Available for the right challenge</div><p className="hero-intro">HELLO, I'M</p><h1 id="hero-title">Himanshu<br /><em>Digari</em></h1><p className="hero-role">TECHNOLOGY LEAD &amp; FRONTEND ENGINEER</p><p className="hero-text">I build scalable, performant and delightful web experiences using modern JavaScript frameworks.</p><TechStack /><div className="hero-actions"><Link className="button primary" to="/work">View My Work <ArrowUpRight size={18} /></Link><a className="button ghost" href="/resume.pdf" download>Download Resume <Download size={17} /></a></div></div></div>
      <section className="hero-stats" aria-label="Career highlights">{heroStats.map(({ value, label, icon: Icon }) => <div className="hero-stat" key={label}><span className="hero-stat-icon"><Icon size={26} strokeWidth={1.8} /></span><span className="hero-stat-copy"><strong>{value}</strong><span>{label}</span></span></div>)}</section>
      <button className="scroll-down" type="button" onClick={scrollToAbout} aria-label="Scroll to About section"><span className="mouse-icon"><span /></span><span>Scroll Down</span><ArrowDown size={18} /></button>
    </section>
    ...
    {rubiksOpen && <RubiksGame onClose={() => setRubiksOpen(false)} />}
  </>;
}
