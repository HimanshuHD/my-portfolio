...
import InteractiveScene from '../components/home/InteractiveScene/InteractiveScene';
  ...
  return <>
    <section className="hero hero-3d" aria-labelledby="hero-title">
      ...
      <section className="hero-stats" aria-label="Career highlights">
        {heroStats.map(({ value, label, icon: Icon }) => <div className="hero-stat" key={label}><span className="hero-stat-icon"><Icon size={26} strokeWidth={1.8} /></span><span className="hero-stat-copy"><strong>{value}</strong><span>{label}</span></span></div>)}
      </section>
